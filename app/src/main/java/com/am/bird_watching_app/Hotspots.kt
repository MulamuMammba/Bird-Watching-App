...
class Hotspots : AppCompatActivity() {
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        mapView.getMapAsync { googleMap ->
            ...
            googleMap.addMarker(
                MarkerOptions().position(LatLng(-26.085334, 27.978662)).title("Randburg Hotspot")
            )
            googleMap.addMarker(
                MarkerOptions().position(LatLng(-26.07143, 28.00251)).title("Sandton Hotspot")
            )
            googleMap.addMarker(
                MarkerOptions().position(LatLng(-26.115, 27.985)).title("Randburg Hotspot")
            )
            googleMap.addMarker(
                MarkerOptions().position(LatLng(-26.111, 28.253)).title("Kempton Park Hotspot")
            )
